...
This report follows the approach proposed by M\"uller et al.~\cite{shapematching} to simulate soft body deformation using shape matching and examines how it can be implemented into Autodesk Maya as a deformer plugin. 
...
After impact, in figure \ref{fig:sfig2}, the mesh will transform into a egg-like shape to later settle down as the original sphere.
...
If the parameters $bounciness$ or $deformation$ is set too high the deformed positions cannot adequately be translated to their goal position, which causes the shape to be flattened upon impact with the ground plane. 
Proper parameter management is therefore required.

Another possible extension to the plugin could be to implement support of object to object collision. 
